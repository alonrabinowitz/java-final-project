...
public class Display {
    ...
    public void drawImageGrid(int[][] f) {
        int piece;
    ...
    public void drawColorGrid(int[][] f) {
        int piece;
        int numcols = f[0].length;
        int numrows = f.length;
    ...
    public void highlightLocation(int row, int col, GameBoard g) {
        if (g.isInGrid(row,col)) {
            Location l = new Location(row,col);
            p.fill(p.color(230, 230, 50, 150));
            p.rect(xCoordOf(l), yCoordOf(l), dx, dy);
        }
    }
